...
public class Maze {
	...
	public Maze(int rows, int columns) {
		System.out.println("In maze"); //test
		this.rows = rows;
		this.columns = columns;
		
		player = new Player();
		generateMaze(rows,columns);
		System.out.println("done with maze");//test
	}
	...
	private void updateEndReachable() {
		if(player.getX() == this.rows -1 && player.getY() == this.columns -1) {
